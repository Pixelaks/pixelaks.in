...
import { getFirestore, doc, getDoc, collection, query, where, orderBy, limit, onSnapshot, setDoc, deleteDoc, serverTimestamp, enableIndexedDbPersistence, arrayRemove } from "https://www.gstatic.com/firebasejs/10.8.1/firebase-firestore.js";
...
let hasStartedInbox = false; // Flag to prevent duplicate inbox listeners

async function fetchTeacherProfile() {
    try {
        // Fetch College Sem Type
        getDoc(doc(db, "colleges", currentCollegeID)).then(snap => {
            if(snap.exists() && snap.data().currentSemesterType) {
                collegeSemesterType = snap.data().currentSemesterType;
            }
        });
...
function ListenToProfile() {
    ...
    profileListener = onSnapshot(teacherDocRef, (snapshot) => {
        ...
        // 🚨 TRIGGER THE INBOX LISTENER NOW THAT WE KNOW THE DEPARTMENT!
...
// ==========================================
// 🚨 NOTIFICATION INBOX ENGINE (Translated from C#)
// ==========================================
let cachedNotifs = [];
...
function startInboxListener() {
    // Exact mapping of Unity's GetSafeTopic()
    ...
    // Exact mapping of Unity's GetMyTopics() for Teachers
    const myTopics = [
        `${safeColID}_ALL`, 
    ...
    // Listen to College-Level Messages
    ...
    inboxListenerUnsub = onSnapshot(query(collection(db, "colleges", currentCollegeID, "inbox_messages"), where("targetTopic", "in", myTopics), orderBy("timestamp", "desc"), limit(30)), (snap) => {
        ...
        // Only show red dot if there are actual messages
        if (snap.docs.length > 0) {
            const redDot = document.querySelector("#btnNotifications .notification-dot");
            if(redDot) redDot.style.display = "block";
        }
        updateNotifUI();
    });

    // Listen to Global Adhyora System Messages
...
function renderNotifications() {
    ...
    // Renders custom styled cards that match the Crimson Teacher Theme
...
// 🚨 SETTINGS & THEMES
// ==========================================
// Inbox Button Override (Hides the red dot on click)
document.getElementById("btnNotifications").addEventListener("click", (e) => {
    switchView(views.notifications.id, e.currentTarget.id);
    let dot = document.querySelector("#btnNotifications .notification-dot");
    if (dot) dot.style.display = "none";
});
document.getElementById("btnMessages").addEventListener("click", (e) => switchView(views.messages.id, e.currentTarget.id));
